    ...
    public static TextureAtlas FromFile(ContentManager content, string fileName)
    {
        ...
        using(Stream stream = TitleContainer.OpenStream(filePath))
        {
            using(XmlReader reader = XmlReader.Create(stream))
            {
                ...
                string texturePath = root.Element("Texture")
                                         .Value;
                ...
                var regions = root.Element("Regions")
                                 ?.Elements("Region");
                ...
                if(regions != null)
                {
                    ...
                    {
                        string name = region.Attribute("name")
                                           ?.Value;
                        int x = int.Parse(region.Attribute("x")
                                               ?.Value ??
                                          "0");
                        int y = int.Parse(region.Attribute("y")
                                               ?.Value ??
                                          "0");
                        int width = int.Parse(region.Attribute("width")
                                                   ?.Value ??
                                              "0");
                        int height = int.Parse(region.Attribute("height")
                                                    ?.Value ??
                                               "0");
                ...
                var animationElements = root.Element("Animations")
                                            .Elements("Animation");
                ...
                if(animationElements != null)
                {
                    foreach(var animationElement in animationElements)
                    {
                        string name = animationElement.Attribute("name")
                                                     ?.Value;
                        float delayInMilliseconds = float.Parse(animationElement.Attribute("delay")
                                                                  ?.Value ??
                                                                "0");
                        ...
                        if(frameElements != null)
                        {
                            ...
                            {
                                string regionName = frameElement.Attribute("region")
                                                                .Value;
